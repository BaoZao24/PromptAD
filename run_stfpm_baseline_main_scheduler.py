#!/usr/bin/env python
"""Scheduler for main STFPM baseline experiments."""
...
GPUS = [0, 2, 3]
ROOT = Path("analysis_outputs/20260709_stfpm_baseline_main")
...
def build_jobs() -> list[tuple[str, list[str]]]:
    return [
        ("per_frequency/self_stfpm", base_cmd("rf_target", "per_frequency", ROOT / "per_frequency" / "self_stfpm")),
        ("per_frequency/public_stfpm", base_cmd("public_rf", "per_frequency", ROOT / "per_frequency" / "public_stfpm")),
        ("4shot/spectrum_stfpm", base_cmd("spectrum", "4shot", ROOT / "4shot" / "spectrum_stfpm")),
    ]
...
def summarize() -> None:
    specs = [
        ("self_rf", "per_frequency", "self_stfpm"),
        ("public_rf", "per_frequency", "public_stfpm"),
        ("spectrum", "4shot", "spectrum_stfpm"),
    ]
    rows = []
    for dataset, sampling, role in specs:
        ...
        rows.append({
            "dataset": dataset,
            "sampling": sampling,
            "stfpm_auc": summary.get("image_auroc_macro"),
    ...
    if not rows:
        return
    out_csv = ROOT / "stfpm_baseline_summary.csv"
    ...
    lines = [
        "# STFPM Main Baseline",
        "",
        "STFPM ResNet18 teacher-student baseline, normal-only training, Image-AUROC.",
        "",
        "| dataset | sampling | STFPM AUROC |",
        "|---|---|---:|",
    ]
    for row in rows:
        lines.append(f"| {row['dataset']} | {row['sampling']} | {float(row['stfpm_auc']):.4f} |")
