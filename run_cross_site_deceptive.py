"""
跨站点测试：用 WeaponMuseum_spectrum 的正常样本训练（k-shot=24），对其他站点做 cross-site 测试，
运行结束后汇总输出结果表。

用法:
    python run_cross_site_deceptive.py
    python run_cross_site_deceptive.py --epochs 100 --gpu-id 1
    python run_cross_site_deceptive.py --dry-run   # 只显示命令不执行
"""
import argparse
import os
import subprocess
...
def print_summary_table(root_dir, seed):
    ...
    for scene in TEST_SCENES:
        val = read_i_roc(root_dir, DATASET, TRAIN_SITE, scene, seed)
        row = f'{scene:<{col_w}}'
        if val is not None:
            vals.append(val)
            row += f'{val:>10.4f}'
...
if __name__ == '__main__':
    ...
    for scene in TEST_SCENES:
        cmd = (
            ...
            f'--vis {vis_str} --seed {args.seed}'
        )
        print(f'\n--- {TRAIN_SITE} -> {scene} ---')
        print(f'运行: {cmd}')
        if not args.dry_run:
            result = subprocess.run(cmd, shell=True)
            run_results[scene] = result.returncode == 0
        else:
            run_results[scene] = None
