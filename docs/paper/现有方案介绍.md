...
频谱异常检测为干扰监测提供支持，但新站点或新频段部署初期通常只能获得少量正常频谱图。正常样本稀缺限制了目标场景模型的训练，而检测器还需要同时识别整体占用变化与局部干扰。为此，本文提出SpectraMemAD，冻结预训练视觉编码器，将场景适配从参数学习转向记忆学习。该方法以视觉Transformer和卷积神经网络特征分别构建正常记忆，通过最近邻查找衡量整体结构与局部纹理的偏离程度。融合规则利用正常patch距离统计量校准两张分数图，在满足门控条件的位置加入CNN局部补充，再取融合图的最大值作为图像级异常分数。在三个射频（RF）数据集的七个少样本设置中，SpectraMemAD的接收者操作特征曲线下面积（AUROC）相对各设置中最强的比较基线提高0.77至10.91个百分点；完整模型相对较强单分支提高0.20至2.88个百分点。
...
将正常特征检索用于少样本频谱异常检测，需要解决三个问题：如何仅以少量正常样本完成新场景适配，如何同时描述整体占用变化与局部干扰，以及如何协调数值尺度和正常patch距离统计量不同的两类异常分数。
...
3. 设计由正常patch距离统计量校准的空间融合规则。在每个位置，仅当CNN距离相对正常patch距离足够高、且校准后的偏离超过ViT时加入局部补充，最后以融合图的最大值作为图像级异常分数。

在三个射频（RF）数据集的全部少样本设置中，SpectraMemAD的接收者操作特征（ROC）曲线下面积（AUROC）均高于所比较基线和两个单分支。

本文其余部分组织如下：第II节定义少样本频谱异常检测问题，第III节介绍所提方法，第IV节报告实验结果，第V节总结全文。
...
冻结的ViT $f_V$提取描述整体时频结构的特征，冻结的CNN $f_C$提取局部纹理特征。两个编码器均保留各空间位置的特征。
...
到当前子集的最近邻距离最大的特征，重复该过程，直至保留$r$比例的特征。最终子集包含不同正常模式的
代表性特征，并作为正常记忆。通过保留具有代表性的正常特征子集，减少记忆存储和最近邻查找开销。
...
模式，该特征更可能异常。将各特征距离按空间位置排列，得到两条分支的patch异常分数图，再按照第III-C节的方法进行校准与空间融合，最后计算图像级分数。
...
其中，$g(x,q)$为门控后的局部修正量，$[u]_+=\max(u,0)$，$\mathbf{1}\{\cdot\}$为指示函数。门控筛选局部偏高的CNN距离，正部差仅保留同一位置上校准后的CNN分数超过ViT分数的部分。IQR因子将修正量转换为ViT距离的量纲，$\alpha$控制补充强度。最后，取融合分数图的最大值作为图像级异常分数：
...
| Public RF[18]                                     | 每个频段1/2/4幅正常频谱图          | 5类异常、3档强度；15个评估单元；图像级指标等权平均           |
...
本文比较八种基线，覆盖统计检测、频谱异常检测和视觉异常检测三类方法。统计基线包括能量检测（energy detection，ED）[1]和正常参考校准的频谱统计集成（support-calibrated spectral ensemble，SCSE）。SCSE将能量、频谱熵、谱平坦度、谱峭度和单元平均恒虚警（CA-CFAR）统计量按正常参考校准，并取最大的正向标准化偏离作为异常分数。频谱基线包括IAD-PER[6]、UDMA[10]、GRETEL[11]和TFAM-AAE[9]，视觉基线包括PatchCore[13]和SPADE[14]。GRETEL将每幅频谱图划分为16个相邻频带节点。需要异常训练样本的监督分类方法不纳入比较。
...
表4表明，SpectraMemAD在三个$k$设置下均取得最高的AUROC和AUPRC以及最低的FPR@95%TPR。相较PatchCore，三个设置下的AUROC分别提高10.91、8.80和5.04个百分点。当正常参考数由1增加到4时，SpectraMemAD的AUROC由81.74%上升至83.74%，FPR@95%TPR由45.69%降至44.12%。其AUROC从1-shot到2-shot、从2-shot到4-shot的增幅分别为1.23和0.77个百分点。相对PatchCore的AUROC优势在1-shot时最大，随后随PatchCore性能的提高而缩小。
...
本文提出少样本频谱异常检测方法SpectraMemAD，将目标场景适配从参数学习转向记忆学习。该方法冻结编码器，利用少量正常频谱图建立全局与局部特征记忆，并依据正常patch距离统计量校准两张分数图后逐位置融合，以融合图的最大值完成图像级评分。
