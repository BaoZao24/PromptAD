...
def load_wideband_pulse(category, k_shot, noise_level='m20db', freq=None, train_category=None):
    ...
    fallback_train_root = os.path.join(WIDEBAND_PULSE_DIR, category, 'normal', noise_level)
    test_normal_root = os.path.join(WIDEBAND_PULSE_DIR, category, 'normal', noise_level)
    test_abnormal_root = os.path.join(WIDEBAND_PULSE_DIR, category, 'abnormal', noise_level)
    groundtruth_root = os.path.join(WIDEBAND_PULSE_DIR, category, 'groundtruth', noise_level)

    train_img_paths = []
    train_labels = []
    train_types = []
    effective_train_root = train_root if os.path.isdir(train_root) else fallback_train_root
    if os.path.isdir(effective_train_root):
        for img_path in sorted(Path(effective_train_root).glob('*.png')):
