...
chirp_signal_classes = ['BinBo', 'CaoChang', 'ShiJianGuangChang', 'TiYuGuan']
...
def load_chirp_signal(category, k_shot, noise_level='m10db', freq=None, train_category=None):
    ...
    test_normal_root = os.path.join(CHIRP_SIGNAL_DIR, map_category_to_dir(category), 'normal', noise_level)
    test_abnormal_root = os.path.join(CHIRP_SIGNAL_DIR, map_category_to_dir(category), 'abnormal', noise_level)
    groundtruth_root = os.path.join(CHIRP_SIGNAL_DIR, map_category_to_dir(category), 'groundtruth', noise_level)
