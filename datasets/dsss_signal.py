...
dsss_classes = ['BinBo', 'CaoChang', 'ShiJianGuangChang', 'TiYuGuan']


NORMAL_DIR = '/mnt/data/wangbei/data/datasets/normal'
DSSS_SIGNAL_DIR = '/mnt/data/wangbei/data/datasets/dsss'


def _extract_time_range(filename: str) -> tuple:
    """从文件名提取时间范围
    例如：BinBo_dsss_m10db_t00000-04000_f100.00-101.50MHz_patch016_normal.png
    返回：(0, 4000)
    """
...
def load_dsss_signal(category, k_shot, noise_level='m10db', freq=None, train_category=None):
    ...
    └── dsss/                           # DSSS 信号测试数据
        └── {category}/
            ├── normal/{noise_level}/           # 正常 DSSS 信号 (测试)
            ├── abnormal/{noise_level}/         # 异常 DSSS 信号 (测试)
    ...
    test_normal_root = os.path.join(DSSS_SIGNAL_DIR, map_category_to_dir(category), 'normal', noise_level)
    test_abnormal_root = os.path.join(DSSS_SIGNAL_DIR, map_category_to_dir(category), 'abnormal', noise_level)
    groundtruth_root = os.path.join(DSSS_SIGNAL_DIR, map_category_to_dir(category), 'groundtruth', noise_level)
