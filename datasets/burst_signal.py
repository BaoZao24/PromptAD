...
BURST_SIGNAL_DIR = '/mnt/data/wangbei/data/datasets/burst'


def _extract_time_range(filename: str) -> tuple:
    """从文件名提取时间范围
    例如：BinBo_burst_m10db_t00000-04000_f100.00-101.50MHz_patch016_normal.png
    返回：(0, 4000)
    """
...
def load_burst_signal(category, k_shot, noise_level='m10db', freq=None, train_category=None,
                      split_mode='legacy', normal_train_ratio=0.75):
    """
    加载 burst 信号数据集用于 burst 信号检测（按场景或频段分开）
    ...
    └── burst/                          # Burst 信号测试数据
        └── {category}/
            ├── normal/{noise_level}/           # 正常 burst 信号 (测试)
            ├── abnormal/{noise_level}/         # 异常 burst 信号 (测试)
    ...
    if split_mode == 'normal_75_25':
        return load_rf_split_dataset(
            BURST_SIGNAL_DIR, category, noise_level=noise_level, freq=freq,
