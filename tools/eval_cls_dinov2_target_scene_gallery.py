#!/usr/bin/env python
"""Evaluate a DINOv2 patch nearest-neighbour gallery on target scenes.

This is the DINOv2 encoder swap of the CLIP-ViT NN-gallery branch. It reuses
the same nearest-neighbour memory-bank scoring idea (row-constrained NN,
coreset, top-ratio aggregation, paired TTA) on DINOv2 patch features, keeping
the target-scene support manifest protocol identical.
"""
...
from train_rf_target_pooled_universal import RFPathDataset
...
@torch.no_grad()
def encode_dino(model, raw_batch, device, rgb_from_bgr=True):
    images = to_dino_input(raw_batch, DINO_TRANSFORM, device, rgb_from_bgr=rgb_from_bgr)
    return extract_patch_features(model, images)


def dino_grid_size(num_patches):
...
def build_vit_patchcore_gallery(model, train_loader, args, device):
    patches = []
    for data, mask, label, name, img_type in tqdm(train_loader, desc="Build DINOv2 NN gallery", leave=False):
        patch_features = encode_dino(model, data, device, rgb_from_bgr=True)
...
def build_vit_nn_gallery_with_rows(
    ...
    for data, mask, label, name, img_type in tqdm(train_loader, desc="Build DINOv2 NN gallery", leave=False):
        ...
        for data_variant in data_variants:
            patch_features = encode_dino(model, data_variant, device, rgb_from_bgr=True)
            gh, gw = dino_grid_size(patch_features.shape[1])
            if grid_h is None:
                grid_h, grid_w = gh, gw
            patches.append(patch_features.reshape(-1, patch_features.shape[-1]))
            row_ids.append(patch_row_indices(patch_features.shape[0], gh, gw, device))
            col_ids.append(patch_col_indices(patch_features.shape[0], gh, gw, device))
    gallery = torch.cat(patches, dim=0)
    rows = torch.cat(row_ids, dim=0)
    cols = torch.cat(col_ids, dim=0)
    if apply_coreset:
        gallery, rows, cols = finalize_vit_nn_gallery(gallery, rows, cols, args)
    return F.normalize(gallery.float(), dim=-1).contiguous(), rows.contiguous(), cols.contiguous(), grid_h, grid_w
...
def compute_patch_map(
    ...
    if visual_features is None or paired_tta_mode != "identity":
        ...
        patch_features = visual_features
    bsz, num_patches, _ = patch_features.shape
    grid_h, grid_w = dino_grid_size(num_patches)
...
def build_gallery_state(model, train_loader, args, device):
    ...
    if args.paired_tta == "none":
        gallery, gallery_rows, gallery_cols, grid_h, grid_w = build_vit_nn_gallery_with_rows(
            model,
            train_loader,
            args,
            device,
        ...
        for mode in _paired_tta_modes(args):
            tta_gallery, tta_rows, tta_cols, gh, gw = build_vit_nn_gallery_with_rows(
                model,
                train_loader,
                args,
                device,
    ...
    row_prototypes = (
        build_row_prototypes(gallery, gallery_rows, grid_h)
        if args.memory_mode == "row_proto"
        else None
    )
    return {
...
def evaluate(
    ...
    for signal, scene, jsr, loader in eval_loaders:
        ...
        for data, mask, label, name, img_type in tqdm(loader, desc=f"Eval DINOv2 NN {signal}/{scene}/{jsr}", leave=False):
            visual_features = encode_dino(model, data, device, rgb_from_bgr=True)
        ...
        row = {
            "method": "dinov2_target_scene_gallery",
            "task": "cls",
            "dataset": signal,
            "scene": scene,
            "jsr": jsr,
            ...
            "dinov2_max_auc": safe_auc(labels, max_np),
        }
        payload = {
            "names": np.asarray(names),
            "labels": labels_np,
            "dinov2_max_scores": max_np,
        }
        if coherent_np is not None:
            row["dinov2_coherent_auc"] = safe_auc(labels, coherent_np)
            payload["dinov2_coherent_scores"] = coherent_np
        for ratio in args.map_top_ratios:
            key = f"{ratio:g}".replace(".", "p")
            values = np.asarray(ratio_scores[ratio], dtype=np.float32)
            row[f"dinov2_top{key}_auc"] = safe_auc(labels, values)
            payload[f"dinov2_top{key}_scores"] = values
...
def parse_args():
    ...
    parser.add_argument("--output-root", default="analysis_outputs/20260814_dinov2_target_scene_gallery")
...
def main():
    ...
    out_root = Path(args.output_root)
    result_path = out_root / "results_cls_dinov2_gallery.csv"
    ...
    readme = [
        "# DINOv2 NN Gallery CLS",
        "",
        "Nearest-neighbour gallery scoring over DINOv2 patch features on target scenes.",
        "",
        "- support protocol: `target_scene`",
        f"- normal sampling: `{args.normal_sampling}`",
        f"- selected normal images: `{len(train_samples)}`",
        f"- encoder: `{args.dino_model}` (input {args.dino_image_size}x{args.dino_image_size})",
