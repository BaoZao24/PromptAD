#!/usr/bin/env python
"""Render the four-view time-plus-background spectral TTA candidate."""
...
OUT = Path("analysis_outputs/20260813_spectral_time_background_tta_preview")
MODES = (
    ("identity", "Original"),
    ("rf_time_shift_up", "Time -4 px"),
    ("rf_time_shift_down", "Time +4 px"),
    ("rf_background_noise_jitter", "Background jitter"),
)
...
def render(name: str) -> Path:
    ...
    fig, axes = plt.subplots(2, len(MODES), figsize=(10.5, 5.5), squeeze=False)
    ...
    fig.suptitle(
        "Four-view spectral Paired-TTA: original, time shifts, and background jitter",
        fontsize=12,
    )
    fig.tight_layout(rect=(0, 0, 1, 0.94), h_pad=1.2)
    OUT.mkdir(parents=True, exist_ok=True)
    path = OUT / f"rf_spectral_time_background_v1_{name}.png"
